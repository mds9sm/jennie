...
> **Version:** 2.0.0 | **Last updated:** 2026-04-14 | **Maintainer:** Sri Maru
...
3. [AI Chat via claude.ai](#3-ai-chat-via-claudeai)
...
Jennie is your data platform intelligence layer — purpose-built for the data team. It operates as two surfaces:

| Surface | URL | Purpose |
|---------|-----|---------|
| **AI Chat via claude.ai** | claude.ai (Team/Enterprise) | Ask questions about your data in plain English |
| **Genie Tools** | genie.example.com | Built-in chat, SQL workspace, glossary, lineage, tasks, reports |

**Genie answers questions on both surfaces, from the same agents and knowledge base.** In claude.ai, Genie connects as two MCP (Model Context Protocol) servers: claude.ai provides the conversation interface, memory, and context management, and Genie provides the data platform intelligence — every table, pipeline, metric, and lineage relationship — through its Knowledge MCP and Data MCP servers. In Genie Tools, the built-in chat at `/` streams the same principal + expert agents directly, and adds what a general-purpose client can't render: inline action cards (query results with auto-charts, lineage, pipeline detail, glossary definitions), shareable session links, PDF export, and one-click scheduling of a question as a recurring report.

**Genie Tools** (genie.example.com) is the web app for day-to-day data work: chat, write and run SQL, manage glossary definitions, visualize lineage, track tasks, and view scheduled reports.
...
You (in claude.ai)
    ...
    │   KB context, lineage, glossary, DOMO metrics,
    │   event schemas, rendered pipeline SQL from MWAA
    │
    └── NP Data MCP          (np-apis.example.com/data-mcp)
        execute_query → Redshift (np + prd via datashare)
        DAG run status + task logs → MWAA

Genie Tools  (genie.example.com)
    Workbench | Glossary | Lineage | Tasks | Reports
```
...
|---------|------------------|
| **Data Engineers** | Ask claude.ai about pipelines, debug DAG failures, review rendered SQL, write and commit transforms in the Workbench, profile tables |
| **Data Analysts** | Ask claude.ai data questions, run queries in the Workbench, look up metric definitions in the Glossary |
| **ML Engineers** | Ask claude.ai about feature tables, data freshness, upstream dependencies for model inputs |
| **New Team Members** | Onboard quickly via claude.ai — ask any question about the data platform instead of searching Confluence/Slack |

### Key Value Proposition

- **Institutional knowledge at your fingertips.** Genie holds the same knowledge as the most senior data engineer — every table, pipeline, metric, and dependency — surfaced through natural conversation, in claude.ai or in Genie's own chat.
...
- **Best-in-class chat, organization-specific knowledge.** claude.ai handles context management, memory, and compaction. Genie provides what claude.ai can't know on its own: the organization's schema, lineage, metrics, and pipeline SQL — and renders it as inline action cards in its own chat.
- **Self-improving.** The knowledge base grows through daily Airflow DAG builds, AI enrichment, and glossary contributions from the team.
...
4. Upon successful login, you land on the Workbench.
...
After completing onboarding, you are assigned a default role and land on the Workbench.
...
## 3. AI Chat via claude.ai

Genie connects to **claude.ai (Team/Enterprise)** as two MCP servers — a knowledge layer and a data layer — so you can ask about your data from inside claude.ai. This section covers that setup (3.1–3.4) and Genie's own built-in chat (3.5), which needs no configuration.

### 3.1 Connecting (Once per User)

1. Open **claude.ai** → **Settings** → **Connectors**
2. Add the Knowledge MCP: `https://dev-apis.example.com/genie/mcp`
3. Sign in via the Okta popup (auto-closes if you're already in Okta)
4. Add the Data MCP: `https://np-apis.example.com/data-mcp`

Both URLs are available with copy buttons in **Genie Tools → Settings → Claude Integration**.

### 3.2 How claude.ai Uses Genie's Tools

When you ask a question, claude.ai calls Genie's MCP tools automatically:

| Step | Tool | What it does |
|------|------|-------------|
| 1 | `get_data_platform_context` | Top 40 tables + schemas, DOMO metrics, glossary defs, SQL patterns (~6K of context) |
| 2 (if needed) | `search_tables`, `get_table_detail`, `get_transform_detail`, etc. | Deep KB lookup for specific tables, DAGs, views, or glossary terms |
| 3 (to query) | `execute_query` (Data MCP) | Runs SELECT against np Redshift; prd tables available via datashare |
| Fallback | `list_tables` + `list_columns` (Data MCP) | Live schema discovery if KB is unavailable |

**Example — "Show me daily cutting users by platform"**
1. claude.ai calls `get_data_platform_context` → gets `fact.cuts_session_master` schema inline
2. claude.ai writes SQL and calls `execute_query` on Data MCP
3. Result: 1 tool call, chart rendered, ~10 seconds

**Example — "What tables feed the onboarding KPI board?"**
1. claude.ai calls `get_data_platform_context` → finds onboarding metrics in glossary context
2. claude.ai calls `get_table_lineage` and `get_transform_detail` for upstream pipeline detail
3. claude.ai synthesizes: "The onboarding KPI board is fed by 3 tables... refreshes daily at..."

### 3.3 Genie MCP Tools Reference

**Knowledge MCP** (`dev-apis.example.com/genie/mcp`):

| Tool | What it returns |
|------|----------------|
| `get_data_platform_context` | Platform summary: top tables, DOMO metrics, glossary, SQL patterns |
| `search_tables` | KB-enriched table search with descriptions, pillar, DOMO links |
| `get_table_detail` | Full schema + profiling + KB annotations |
| `get_table_lineage` | Upstream/downstream dependency graph |
| `search_transforms` | DAG index with business context |
| `get_transform_detail` | Rendered Jinja SQL from MWAA task logs |
| `get_view_detail` | View SQL + DOMO dataset link |
| `glossary_lookup` | Business definitions, metric formulas |
| `analyze_domo_dataset` | DuckDB analysis on DOMO S3 exports |
| `domo_search` / `domo_dataset_info` / `domo_dashboards` | DOMO REST API metadata |
| `search_events` / `get_event_schema` | 558 ProductApp event schemas |
| `repo_search` / `github_file` | Airflow repo search |

**Data MCP** (`np-apis.example.com/data-mcp`):

| Tool | What it does |
|------|-------------|
| `execute_query(sql)` | SELECT against Redshift (read-only enforced) |
| `list_schemas()` | Available databases and schemas |
| `list_tables(schema)` | Tables in a schema with column counts |
| `list_columns(table)` | Column names, types, nullable |
| `get_mwaa_dag_runs(dag_id)` | Recent DAG run status + timestamps |
| `get_mwaa_task_log(dag_id, run_id, task_id)` | Task execution log |

### 3.4 System Prompt for Admins

Add this once to **claude.ai Team Settings → System Prompt**:
...
3. If Genie Knowledge is unavailable, use NP Data list_tables + list_columns
   for schema discovery, then write SQL directly.
...
### 3.5 Built-in Chat (genie.example.com)

Genie Tools opens on its own chat at `/`. It needs no setup — you're already signed in — and
runs the same principal + expert agents that back the MCP servers.

**Investigation trail.** Each tool call appears as it happens (searching tables, running SQL,
consulting the Data or Redshift Expert) with a spinner while it runs and a checkmark when it
finishes, so you can see how an answer was reached.

**Action cards.** Tool results render inline beneath the answer rather than as raw JSON:

| Card | Shows |
|---|---|
| Query Results | Columns and rows, auto-charted (bar/line/pie), always expanded. Only the last query of a turn is kept |
| Table | Schema, columns, profile stats, lineage links |
| Pipeline | DAG detail, rendered SQL, run history |
| Lineage | Upstream/downstream tables, clickable |
| Definition | Glossary term, formula, source tables |
| Code | Repo search results |

**Sessions.** Conversations save automatically and appear in the sidebar for 30 days.
Double-click to rename, the link icon copies a read-only share URL (`/?shared=<id>`), the X
deletes. Long conversations are summarized after 10 messages to keep context in budget — the
message counter under the input shows when this happens.

**Attachments.** The paperclip accepts images (passed to the model directly) and documents
(.txt/.md/.csv/.yaml/.json/.pdf/.docx — text extracted server-side, capped at 12K characters).

**Per-response actions.** Copy, export to PDF, schedule as a recurring report (section 4), or
create a task / request support (section 9).

**Knowledge capture.** Genie writes back what it learns from chat: `💡 KB Improvement` markers
open KB-gap tasks (deduplicated within 24 hours, max 3 per response) and `📝 Correction`
markers create glossary drafts assigned to you for review.
